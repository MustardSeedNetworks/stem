...
- UI config is **strict**, but slightly more relaxed than Seed:
  - `noImplicitBoolean`, `noNegationElse`, `noNestedTernary`, `useAtIndex`, `useBlockStatements`,
    `useDefaultSwitchClause`, `useSimplifiedLogicExpression`, `noVoid`, `noForEach` set to `off`.
