...
| `security` | Security scans | govulncheck (hard gate), gosec via golangci-lint (blocking gate; G103/G204/G304 are SARIF-only via the standalone step), npm audit, gitleaks, Trivy |
...
make security-backend  # gosec (blocking via golangci-lint; G103/G204/G304 SARIF-only) + govulncheck
```
